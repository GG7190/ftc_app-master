...
@TeleOp(name="SuctionDrive",group="TeleOp")
public class SuctionDrive extends LinearOpMode {
    ...
    public void runOpMode(){
        ...
        while (opModeIsActive()) {
            ...
            if (gamepad2.right_bumper) {
                robot.suctionArm.setPosition(robot.RCLAW_OPEN);
            }
            //Closes the Claws
            if (gamepad2.left_bumper) {
                robot.suctionArm.setPosition(robot.RCLAW_CLOSE);
            }
           ...
           /* if (gamepad2.left_trigger > 10)
            ...
            if(gamepad2.x)
            {
                if(robot.pump.getPower() > 0.0)
                {
                    robot.pump.setPower(0.0);
                }

                else
                {
                    robot.pump.setPower(1.0);
                }
            }
